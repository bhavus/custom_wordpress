<?php
/*
Plugin Name: Munshaip Only Database Backup
Plugin URI: https://xyz.com/
Description: only databse backup 
...
if ( ! defined( 'MUNSP_ROWS_PER_SEGMENT' ) ) {
	define( 'MUNSP_ROWS_PER_SEGMENT', 100 );
}

/**
 * Set MUNSP_MOD_EVASIVE_OVERRIDE to true
 * and increase MUNSP_MOD_EVASIVE_DELAY
 * if the backup stops prematurely.
 */
// define('MUNSP_MOD_EVASIVE_OVERRIDE', false);
if ( ! defined( 'MUNSP_MOD_EVASIVE_DELAY' ) ) {
	define( 'MUNSP_MOD_EVASIVE_DELAY', '500' );
}
...
class wpdbBackup {
	...
	function module_check() {
		...
		if ( defined( 'MUNSP_MOD_EVASIVE_OVERRIDE' ) && true === MUNSP_MOD_EVASIVE_OVERRIDE ) {
			return true;
		}

		if ( ! defined( 'MUNSP_MOD_EVASIVE_OVERRIDE' ) || false === MUNSP_MOD_EVASIVE_OVERRIDE ) {
			return false;
		}
	...
	function build_backup_script() {
		...
		foreach ( $tables as $table ) {
			...
			$rec_segments = ceil( $rec_count / MUNSP_ROWS_PER_SEGMENT );
			$table_count  = 0;
			if ( $this->module_check() ) {
				$delay      = "setTimeout('";
				$delay_time = "', " . (int) MUNSP_MOD_EVASIVE_DELAY . ')';
			} else {
				$delay = $delay_time = ''; }
			do {
	...
	function backup_table( $table, $segment = 'none' ) {
		...
		if ( ( $segment == 'none' ) || ( $segment >= 0 ) ) {
			...
			if ( $segment == 'none' ) {
				$row_start = 0;
				$row_inc   = MUNSP_ROWS_PER_SEGMENT;
			} else {
				$row_start = $segment * MUNSP_ROWS_PER_SEGMENT;
				$row_inc   = MUNSP_ROWS_PER_SEGMENT;
			}
	...
	function backup_menu() {
		...
		<div id="wpdb" class='wrap'>
			<div class="header">
				...
				<h2 class="title"><?php _e( 'MUNSHAIP ONLY DATABASE BACKUP', 'wp-db-backup' ); ?></h2>
			</div>
