var munshaip_settings = (function($, window, document) {
	...
	var OPTION_ROW_HTML  = '<tr>';
		...
		OPTION_ROW_HTML += '<td class="action-cell"><a href="javascript:void(0)" onclick="munshaipAddNewOptionRow(this)" class="btn btn-blue" title="Add new option">+</a></td>';
		OPTION_ROW_HTML += '<td class="action-cell"><a href="javascript:void(0)" onclick="munshaipRemoveOptionRow(this)" class="btn btn-red"  title="Remove option">x</a></td>';
	...
	*------------------------------------*/
    ...
    $( "#munshaip_new_field_form_pp" ).dialog({
	  	modal: true,
		width: 800,
		//height: 400,
		resizable: false,
				...
				},
		{
			...
			click: function() {
				$('<input>').attr({
					...
					id: 'foo',
					name: 'save_fields'
				}).appendTo('#munshaip_checkout_fields_form');
				var result = munshaip_add_new_row( this );
				
				var form = $("#munshaip_checkout_fields_form");
				if(result){
	...
	$( "#munshaip_edit_field_form_pp" ).dialog({
	  	modal: true,
		width: 800,
		//height: 400,
		resizable: false,
				...
				},
		{
			...
			click: function() {
				...
				var result = munshaip_update_row( this );
	...
	$('select.munshaip-enhanced-multi-select').select2({
	...
	$( ".munshaip_remove_field_btn" ).click( function() {
		var form =  $(this.form);		
		
		$('#munshaip_checkout_fields tbody input:checkbox[name=select_field]:checked').each(function () {
			$(this).closest('tr').remove();
	  	});	  	
	});
	
	$('#munshaip_checkout_fields tbody').sortable({
		items:'tr',
		cursor:'move',
		axis:'y',
		handle: 'td.sort',
	...
	$("#munshaip_checkout_fields tbody").on("sortstart", function( event, ui ){
		ui.item.css('background-color','#f6f6f6');										
	});
	
	$("#munshaip_checkout_fields tbody").on("sortstop", function( event, ui ){
		ui.item.removeAttr('style');
		munshaip_prepare_field_order_indexes();
	});
	...
	_saveCustomFieldForm = function saveCustomFieldForm(loaderPath,donePath){
		var formData = $('#munshaip_custom_options_form').serializeArray();
	...
	var data = {
			...
			$("body").append("<div class='munshaip_spinner'><img src='"+loaderimg+"' /></div>");

			},
			success: function(data){
				var loaderimg = donePath;
				$("body .munshaip_spinner").html("<img src='"+loaderimg+"' />");
				
				setTimeout(function(){
				 $("body .munshaip_spinner").remove();
				}, 500)
	...
	function setup_enhanced_multi_select(form){
		form.find('select.munshaip-enhanced-multi-select2').each(function(){
	...
	_openNewFieldForm = function openNewFieldForm(tabName){
		...
		var form = $("#munshaip_new_field_form_pp");
		munshaip_clear_form(form);
		
		form.find("select[name=ftype]").change();
		form.find("select[name=fclass]").val('form-row-wide');
		
	  	$( "#munshaip_new_field_form_pp" ).dialog( "open" );
	}
	
	function munshaip_add_new_row(form){
		...
		var index = $('#munshaip_checkout_fields tbody tr').size();
		...
		$('#munshaip_checkout_fields tbody tr:last').after(newRow);
		return true;
	}
	...
	*----------------------------------------------*/
	...
	var OP_AND_HTML  = '<label class="thpl_logic_label">AND</label>';
		OP_AND_HTML += '<a href="javascript:void(0)" onclick="munshaipRemoveRuleRow(this)" class="thpl_logic_link" title="Remove">X</a>';
	var OP_OR_HTML   = '<tr class="thpl_logic_label_or"><td colspan="4" align="center">OR</td></tr>';
	
	var OP_HTML = '<a href="javascript:void(0)" class="thpl_logic_link" onclick="munshaipAddNewConditionRow(this, 2)" title="">+</a>';
		OP_HTML += '<a href="javascript:void(0)" onclick="munshaipRemoveRuleRow(this)" class="thpl_logic_link" title="Remove">X</a>';
	
	var CONDITION_HTML = '', CONDITION_SET_HTML = '', CONDITION_SET_HTML_WITH_OR = '', RULE_HTML = '', RULE_SET_HTML = '';
	
	$(function() {
		
	    CONDITION_HTML  = '<tr class="munshaip_condition">';
	    ...
	    CONDITION_SET_HTML  = '<tr class="munshaip_condition_set_row"><td>';
		CONDITION_SET_HTML += '<table class="munshaip_condition_set" width="100%" style=""><tbody>'+CONDITION_HTML+'</tbody></table>';
		CONDITION_SET_HTML += '</td></tr>';
		
	    CONDITION_SET_HTML_WITH_OR  = '<tr class="munshaip_condition_set_row"><td>';
		CONDITION_SET_HTML_WITH_OR += '<table class="munshaip_condition_set" width="100%" style=""><thead>'+OP_OR_HTML+'</thead><tbody>'+CONDITION_HTML+'</tbody></table>';
		CONDITION_SET_HTML_WITH_OR += '</td></tr>';
	
	    RULE_HTML  = '<tr class="munshaip_rule_row"><td>';
		RULE_HTML += '<table class="munshaip_rule" width="100%" style=""><tbody>'+CONDITION_SET_HTML+'</tbody></table>';
		RULE_HTML += '</td></tr>';	
		
	    RULE_SET_HTML  = '<tr class="munshaip_rule_set_row"><td>';
		RULE_SET_HTML += '<table class="munshaip_rule_set" width="100%"><tbody>'+RULE_HTML+'</tbody></table>';
		RULE_SET_HTML += '</td></tr>';
	});
	
	function prepareRuleOperatorSet(value){
		var html = '<select name="i_rule_operator" style="width:200px;" value="'+ value +'" onchange="munshaipRuleOperatorChangeListner(this)">';
	...
	function prepareRuleOperandTypeSet(value){
		var html = '<select name="i_rule_operand_type" style="width:200px;" onchange="munshaipRuleOperandTypeChangeListner(this)" value="'+ value +'">';
		html += '<option value=""></option>';
	...
	function prepareRuleOperandSet(operand_type, operand, operator){
		...
		if(operand_type === "product"){
			html += $("#munshaip_product_select").html();
			
		}else if(operand_type === "category"){
			html += $("#munshaip_product_cat_select").html();
			
		}else if(operator === "user_role_eq" || operator === "user_role_ne"){
			html += $("#munshaip_user_role_select").html();
	...
	_add_new_rule_row = function add_new_rule_row(elm, op){
		...
		if(op == 1){
			var conditionSetTable = $(elm).closest('.munshaip_condition_set');
			var conditionSetSize  = conditionSetTable.find('tbody tr.munshaip_condition').size();
			
			if(conditionSetSize > 0){
				$(elm).closest('td').html(OP_AND_HTML);
				conditionSetTable.find('tbody tr.munshaip_condition:last').after(CONDITION_HTML);
			}else{
				conditionSetTable.find('tbody').append(CONDITION_HTML);
			}
		}else if(op == 2){
			var ruleTable = $(elm).closest('.munshaip_rule');
			var ruleSize  = ruleTable.find('tbody tr.munshaip_condition_set_row').size();
			
			if(ruleSize > 0){
				ruleTable.find('tbody tr.munshaip_condition_set_row:last').after(CONDITION_SET_HTML_WITH_OR);
			}else{
	...
	_remove_rule_row = function remove_rule_row(elm){
		var ctable = $(elm).closest('table.munshaip_condition_set');
		var rtable = $(elm).closest('table.munshaip_rule');
		
		$(elm).closest('tr.munshaip_condition').remove();
		
		var cSize = ctable.find('tbody tr.munshaip_condition').size();
		if(cSize == 0){
			ctable.closest('tr.munshaip_condition_set_row').remove();
		}else{
			ctable.find('tbody tr.munshaip_condition:last').find('td.actions').html(OP_HTML);	
		}
		
		rSize = rtable.find('tbody tr.munshaip_condition_set_row').size();
		if(cSize == 0 && rSize == 0){
	...
	function get_conditional_rules(elm, ajaxFlag){
		...
		if(ajaxFlag){
			rulesTable = $(elm).find(".munshaip_conditional_rules_ajax tbody");
		}else{
			rulesTable = $(elm).find(".munshaip_conditional_rules tbody");	
		}
		
		var conditionalRules = [];
		rulesTable.find("tr.munshaip_rule_set_row").each(function() {
			var ruleSet = [];
			$(this).find("table.munshaip_rule_set tbody tr.munshaip_rule_row").each(function() {
				var rule = [];															 
				$(this).find("table.munshaip_rule tbody tr.munshaip_condition_set_row").each(function() {
					var conditions = [];
					$(this).find("table.munshaip_condition_set tbody tr.munshaip_condition").each(function() {
						condition = {};
						if(ajaxFlag){
	...
	function populate_conditional_rules(form, conditionalRulesJson, ajaxFlag){
		...
		if(ajaxFlag){
			conditionalRulesTable = form.find(".munshaip_conditional_rules_ajax tbody");
		}else{
			conditionalRulesTable = form.find(".munshaip_conditional_rules tbody");
		}
		...
		if(conditionalRulesHtml){
			...
			conditionalRulesTable.find('tr.munshaip_condition').each(function(){
	...
	function populate_rule_set_html(ruleHtml){
		var html = '';
		if(ruleHtml){
			html += '<tr class="munshaip_rule_set_row"><td><table class="munshaip_rule_set" width="100%"><tbody>';
			html += ruleHtml;
	...
	function populate_rule_html(conditionSetHtml){
		var html = '';
		if(conditionSetHtml){
			html += '<tr class="munshaip_rule_row"><td><table class="munshaip_rule" width="100%" style=""><tbody>';
	...
	function populate_condition_set_html(conditionsHtml, firstRule){
		...
		if(conditionsHtml){
			if(firstRule){
				html += '<tr class="munshaip_condition_set_row"><td><table class="munshaip_condition_set" width="100%" style=""><tbody>';
				html += conditionsHtml;
				html += '</tbody></table></td></tr>';
			}else{
				html += '<tr class="munshaip_condition_set_row"><td><table class="munshaip_condition_set" width="100%" style=""><thead>'+OP_OR_HTML+'</thead><tbody>';
				html += conditionsHtml;
				html += '</tbody></table></td></tr>';
			}
		}
	...
	function populate_condition_html(condition, lastRow, ajaxFlag){
		...
		if(condition){
			if(ajaxFlag){
				...
				html += '<tr class="munshaip_condition">';
	...
	function ruleOperatorChangeAction(elm, ignoreUserRole){
		...
		if(operator === 'user_role_eq' || operator === 'user_role_ne'){
			if(ignoreUserRole){
				...
				var target = condition_row.find("td.thpladmin_rule_operand");
				target.html( $("#munshaip_user_role_select").html() );
				setup_enhanced_multi_select(condition_row);
			}
	...
	*---------------------------------------------------*/
	...
	var OP_AND_HTML_AJAX  = '<label class="thpl_logic_label">AND</label>';
		OP_AND_HTML_AJAX += '<a href="javascript:void(0)" onclick="munshaipRemoveRuleRowAjax(this)" class="thpl_logic_link" title="Remove">X</a>';
	
	var OP_HTML_AJAX = '<a href="javascript:void(0)" class="thpl_logic_link" onclick="munshaipAddNewConditionRowAjax(this, 2)" title="">+</a>';
		OP_HTML_AJAX += '<a href="javascript:void(0)" onclick="munshaipRemoveRuleRowAjax(this)" class="thpl_logic_link" title="Remove">X</a>';
	...
	$(function() {
		...
		CONDITION_HTML_AJAX  = '<tr class="munshaip_condition">';
		...
		CONDITION_SET_HTML_AJAX  = '<tr class="munshaip_condition_set_row"><td>';
		CONDITION_SET_HTML_AJAX += '<table class="munshaip_condition_set" width="100%" style=""><tbody>'+CONDITION_HTML_AJAX+'</tbody></table>';
		CONDITION_SET_HTML_AJAX += '</td></tr>';
			
		CONDITION_SET_HTML_WITH_OR_AJAX  = '<tr class="munshaip_condition_set_row"><td><table class="munshaip_condition_set" width="100%" style="">';
		...
		RULE_HTML_AJAX  = '<tr class="munshaip_rule_row"><td>';
		RULE_HTML_AJAX += '<table class="munshaip_rule" width="100%" style=""><tbody>'+CONDITION_SET_HTML_AJAX+'</tbody></table>';
		RULE_HTML_AJAX += '</td></tr>';	
		
		RULE_SET_HTML_AJAX  = '<tr class="munshaip_rule_set_row"><td>';
		RULE_SET_HTML_AJAX += '<table class="munshaip_rule_set" width="100%"><tbody>'+RULE_HTML_AJAX+'</tbody></table>';
		RULE_SET_HTML_AJAX += '</td></tr>';
	});
			   
	function prepareRuleOperatorSetAjax(value){
		var html = '<select name="i_rule_operator" style="width:200px;" value="'+ value +'" onchange="munshaipRuleOperatorChangeListnerAjax(this)" >';
		html += '<option value=""></option>';
	...
	function prepareRuleOperandSetAjax(value){
		...
		html += $("#munshaip_checkout_fields_select").html();
		return html;
	}
	...
	_add_new_rule_row_ajax = function addNewRuleRowAjax(elm, op){
		...
		if(op == 1){
			...
			if(conditionSetSize > 0){
				...
				conditionSetTable.find('tbody tr.munshaip_condition:last').after(CONDITION_HTML_AJAX);
			}else{
			...
			if(ruleSize > 0){
				ruleTable.find('tbody tr.munshaip_condition_set_row:last').after(CONDITION_SET_HTML_WITH_OR_AJAX);
			}else{
	...
	_remove_rule_row_ajax = function removeRuleRowAjax(elm){
		...
		if(cSize == 0){
			...
			ctable.find('tbody tr.munshaip_condition:last').find('td.actions').html(OP_HTML_AJAX);	
		}
	...
	*---------------------------------------------------*/
	...
	_openEditFieldForm = function openEditFieldForm(elm, rowId){
		...
		var form = $("#munshaip_edit_field_form_pp");
		...
		form.find("select[name=ftype]").change();
		$( "#munshaip_edit_field_form_pp" ).dialog( "open" );
	...
	function munshaip_update_row(form){
		...
		var row = $('#munshaip_checkout_fields tbody').find('.row_'+rowId);
	...
	*--------------------------------------*/
	...
	_removeSelectedFields = function removeSelectedFields(){
		$('#munshaip_checkout_fields tbody tr').removeClass('strikeout');
		$('#munshaip_checkout_fields tbody input:checkbox[name=select_field]:checked').each(function () {
			//$(this).closest('tr').remove();
	...
	_enableDisableSelectedFields = function enableDisableSelectedFields(enabled){
		$('#munshaip_checkout_fields tbody input:checkbox[name=select_field]:checked').each(function () {
			var row = $(this).closest('tr');
			if(enabled == 0){
				if(!row.hasClass("munshaip-disabled")){
					row.addClass("munshaip-disabled");
				}
			}
				...
				row.removeClass("munshaip-disabled");				
			}
	...
	function munshaip_clear_form( form ){
		form.find('.err_msgs').html('');
	...
	*------------------------------------*/
	...
	function populate_options_list(elm, optionsJson){
			...
			try{
				...
				if(optionsList){
					jQuery.each(optionsList, function() {
						...
						html +='<td class="action-cell"><a href="javascript:void(0)" onclick="munshaipAddNewOptionRow(this)" class="btn btn-blue" title="Add new option">+</a></td>';
						html += '<td class="action-cell"><a href="javascript:void(0)" onclick="munshaipRemoveOptionRow(this)" class="btn btn-red"  title="Remove option">x</a></td>';
		...
		var optionsTable = $(elm).find(".munshaip-option-list tbody");
		if(optionsHtml){
	...
	*------------------------------------*/
	...
	function munshaip_prepare_field_order_indexes() {
		$('#munshaip_checkout_fields tbody tr').each(function(index, el){
			$('input.f_order', el).val( parseInt( $(el).index('#munshaip_checkout_fields tbody tr') ) );
		});
	};
	...
	_selectAllCheckoutFields = function selectAllCheckoutFields(elm){
		...
		$('#munshaip_checkout_fields tbody input:checkbox[name=select_field]').prop('checked', checkAll);
	}
...
function saveCustomFieldForm(loaderPath,donePath){
	munshaip_settings.saveCustomFieldForm(loaderPath,donePath);		
}

function saveFieldForm(tabName,pluginPath){
	munshaip_settings.saveFieldForm(tabName,pluginPath);		
}

function munshaipRuleOperatorChangeListner(elm){
	munshaip_settings.ruleOperatorChangeListner(elm);
}

function munshaipFieldTypeChangeListner(elm){	
	munshaip_settings.fieldTypeChangeListner(elm);
}


function munshaipRuleOperandTypeChangeListner(elm){
	munshaip_settings.ruleOperandTypeChangeListner(elm);
}

function munshaipAddNewConditionRow(elm, op){
	munshaip_settings.add_new_rule_row(elm, op);
}
function munshaipAddNewConditionRowAjax(elm, op){
	munshaip_settings.add_new_rule_row_ajax(elm, op);
}

function munshaipRemoveRuleRow(elm){
	munshaip_settings.remove_rule_row(elm);
}
function munshaipRemoveRuleRowAjax(elm){
	munshaip_settings.remove_rule_row_ajax(elm);
}

function munshaipRuleOperatorChangeListnerAjax(elm){
	munshaip_settings.ruleOperatorChangeListnerAjax(elm);
}

function openNewFieldForm(tabName){
	munshaip_settings.openNewFieldForm(tabName);		
}

function openEditFieldForm(elm, rowId){
	munshaip_settings.openEditFieldForm(elm, rowId);		
}
	
function removeSelectedFields(){
	munshaip_settings.removeSelectedFields();
}

function enableSelectedFields(){
	munshaip_settings.enableDisableSelectedFields(1);
}

function disableSelectedFields(){
	munshaip_settings.enableDisableSelectedFields(0);
}


function munshaipSelectAllCheckoutFields(elm){
	munshaip_settings.selectAllCheckoutFields(elm);
}

function munshaipAddNewOptionRow(elm){
	munshaip_settings.addNewOptionRow(elm);
}
function munshaipRemoveOptionRow(elm){
	munshaip_settings.removeOptionRow(elm);
}


function munshaipPriceTypeChangeListener(elm){
	munshaip_settings.priceTypeChangeListener(elm);
}
